...
def make_connector(mock_providers):
    ...
    def _make(**config_kwargs):
        config = SpeakKokoroTTSConfig(**config_kwargs)
...
class TestGreetingConversationKokoroConnector:
    """Test the Kokoro greeting conversation connector."""
    ...
    def test_tick_skips_during_tts_activity(self, connector, mock_providers):
        ...
        with patch(
            "actions.greeting_conversation.connector.greeting_conversation_kokoro.logging"
        ):
            connector.tick()
